    ...
    <div class="flex-1 p-4 md:p-6 overflow-y-auto" ref="chatContainer">
      <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div v-if="!currentHistory" class="flex flex-col items-center justify-center h-full py-10 text-center">
          <i class="fa fa-globe text-5xl text-gray-300 mb-4"></i>
          <h3 class="text-lg font-medium mb-2">开始新的对话</h3>
          <p class="text-gray-500 text-sm">输入产品信息开始GEO优化软文生成</p>
        </div>
      ...
      <div class="max-w-3xl mx-auto">
        <form @submit.prevent="sendMessage" class="flex flex-col gap-3">
          <textarea
            ...
            placeholder="输入产品信息，如产品名称、特点、目标受众等..."
...
const sendMessage = async () => {
  const history = currentHistory.value;
    ...
    // GEO优化API
    const apiEndpoint = 'geo/chat';
    const requestBody = {
      text: newMessage,
      user_id: 'user_12345',
      agent_id: 'geo_optimizer',
      session_id: `geo_session_${Date.now()}`,
      enable_memory: true,
