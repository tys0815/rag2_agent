...
class ToolRegistry:
            ...
            if expanded_tools:
                ...
                for sub_tool in expanded_tools:
                    if sub_tool.name in self._tools:
                        print(f"[WARN] 警告：工具 '{sub_tool.name}' 已存在，将被覆盖。")
                    self._tools[sub_tool.name] = sub_tool
                print(f"[OK] 工具 '{tool.name}' 已展开为 {len(expanded_tools)} 个独立工具")
                return

        # 普通工具或不展开的工具
        if tool.name in self._tools:
            print(f"[WARN] 警告：工具 '{tool.name}' 已存在，将被覆盖。")
    ...
    def register_function(self, name: str, description: str, func: Callable[[str], str]) -> None:
        ...
        if name in self._functions:
            print(f"[WARN] 警告：工具 '{name}' 已存在，将被覆盖。")
